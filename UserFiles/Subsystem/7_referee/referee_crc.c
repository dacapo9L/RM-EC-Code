...
static const uint16_t k_crc16_table[256] = {
...
uint8_t Referee_CRC8_Verify(const uint8_t *data, uint16_t len_with_crc) {
  if (data == 0 || len_with_crc < 2u) {
    return 0u;
  }
  return Referee_CRC8_Calc(data, (uint16_t)(len_with_crc - 1u), 0xFFu) ==
         data[len_with_crc - 1u];
}
...
uint16_t Referee_CRC16_Calc(const uint8_t *data, uint32_t len, uint16_t init) {
  ...
  for (i = 0; i < len; ++i) {
    crc = (uint16_t)((crc >> 8) ^
                     k_crc16_table[(crc ^ (uint16_t)data[i]) & 0x00FFu]);
  }
  return crc;
}
...
uint8_t Referee_CRC16_Verify(const uint8_t *data, uint32_t len_with_crc) {
  uint16_t expected;
  uint16_t actual;
  if (data == 0 || len_with_crc < 3u) {
    return 0u;
  }
  expected = Referee_CRC16_Calc(data, len_with_crc - 2u, 0xFFFFu);
  actual = (uint16_t)data[len_with_crc - 2u] |
           (uint16_t)((uint16_t)data[len_with_crc - 1u] << 8);
