...
class ArmorDetector:
    ...
    def _extract_light_bars(self, gray: np.ndarray, mask: np.ndarray) -> List[LightBar]:
        ...
        for c in contours:
            ...
            if fill_ratio < self.cfg.min_light_fill_ratio:
                continue
    ...
    def _pair_score(
        ...
        if h <= 0 or w <= 0:
            return None
        armor_aspect_ratio = w / max(float(h), 1e-6)
        if armor_aspect_ratio < self.cfg.min_armor_aspect_ratio:
            return None
        if armor_aspect_ratio > self.cfg.max_armor_aspect_ratio:
            return None

        # Armor core should usually be dimmer than the two bright light bars.
        x1 = int(x + 0.2 * w)
        x2 = int(x + 0.8 * w)
        y1 = int(y + 0.2 * h)
        y2 = int(y + 0.8 * h)
        ...
        if inner_roi.size == 0:
            return None
        inner_mean = float(np.mean(inner_roi))
        pair_light_mean = 0.5 * (left.mean_intensity + right.mean_intensity)
        inner_to_light_ratio = inner_mean / max(pair_light_mean, 1.0)
        if inner_to_light_ratio > self.cfg.max_inner_to_light_ratio:
            return None
        ...
        inner_score = 1.0 - min(1.0, inner_to_light_ratio / max(self.cfg.max_inner_to_light_ratio, 1e-6))
        ...
        if self._last_center is not None and self._lost_count <= self.cfg.max_lost_hold_frames:
            track_dist = np.hypot(
                cx - float(self._last_center[0]),
                cy - float(self._last_center[1]),
            ) / max(image_diag, 1.0)
            if track_dist <= self.cfg.track_max_dist_ratio:
                track_score = 1.0 - min(1.0, track_dist / max(self.cfg.track_max_dist_ratio, 1e-6))
                score += self.cfg.track_weight * track_score
    ...
    def _detect_from_mask(
        ...
        for i in range(len(bars)):
            for j in range(i + 1, len(bars)):
                candidate = self._pair_score(gray, bars[i], bars[j], image_center, image_diag)
